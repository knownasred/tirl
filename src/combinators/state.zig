...
pub const ParserState = struct {
    ...
    pub fn init(data: []const u8) ParserState {
        return .{
            .data = data,
            .pos = 0,
            .line = 0,
    ...
    pub fn fromFile(alloc: std.mem.Allocator, path: []const u8) !ParserState {
    ...
    pub fn deinit(self: *ParserState, alloc: std.mem.Allocator) void {
    ...
    pub fn checkpoint(self: *const ParserState) Checkpoint {
        return .{ .pos = self.pos, .line = self.line, .col = self.col };
    }

    /// Restore parser state to a previously saved checkpoint.
    pub fn restore(self: *ParserState, cp: Checkpoint) void {
        self.pos = cp.pos;
        self.line = cp.line;
        self.col = cp.col;
    }
    ...
    pub fn isEof(self: *const ParserState) bool {
